...
st.markdown("<h2 style='text-align: center; color: orange;'>User Input for Segment Prediction </h2>", unsafe_allow_html=True)
...
progress_text='Operation is progress'
progress_bar=st.progress(0,progress_text)
for i in range (100):
     time.sleep(1)
     progress_bar.progress(i+1,text=progress_text)

tab01,tab02,tab03,tab04= st.tabs(['1','2','3','4'])
with tab01:
     st_shap(shap.plots.waterfall(shap_values[0]), height=600, width=800)
with tab02:
    st_shap(shap.plots.beeswarm(shap_values), height=600, width=800)
with tab03:
    st_shap(shap.force_plot(explainer1.expected_value, shap_values1[0,:], X_display.iloc[0,:]), height=300, width=1000)

with tab04:
    st_shap(shap.force_plot(explainer1.expected_value, shap_values1[:1000,:], X_display.iloc[:1000,:]), height=400, width=1000)
